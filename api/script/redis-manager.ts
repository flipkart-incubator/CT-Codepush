...
class PromisifiedRedisClient {
  ...
  constructor(redisClient: redis.RedisClient) {
    ...
    this.execBatch = function execBatch(redisBatchClient: any) {
      return q.Promise((resolve, reject) => {
        redisBatchClient.exec((err: Error, results: any[]) => {
          if (err) reject(err);
          else resolve(results);
        });
      });
    };
    this.exists = promisify(redisClient.exists).bind(redisClient);
    ...
    this.hincrby = promisify(redisClient.hincrby).bind(redisClient);
    this.hset = promisify(redisClient.hset).bind(redisClient);
    this.ping = promisify(redisClient.ping).bind(redisClient);
    this.quit = promisify(redisClient.quit).bind(redisClient);
    this.select = promisify(redisClient.select).bind(redisClient);
