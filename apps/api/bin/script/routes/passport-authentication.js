...
class PassportAuthentication {
    ...
    static GOOGLE_PROVIDER_NAME = "google";
    ...
    getRouter() {
        ...
        q.resolve(storage.getAccessSecret(process.env.GOOGLE_CLIENT_ID) || "")
            .then((googleClientId) => {
            q.resolve(storage.getAccessSecret(process.env.GOOGLE_CLIENT_SECRET) || "")
                .then((googleClientSecret) => {
                const isGoogleAuthenticationEnabled = !!this._serverUrl && !!googleClientId && !!googleClientSecret;
    ...
    setupGoogleRoutes(router, googleClientId, googleClientSecret) {
