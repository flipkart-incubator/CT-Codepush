...
export class PassportAuthentication {
  ...
  public getRouter(): Router {
    ...
    q.resolve(storage.getAccessSecret(process.env.GOOGLE_CLIENT_ID) || "" as string)
  .then((googleClientId) => {
    q.resolve(storage.getAccessSecret(process.env.GOOGLE_CLIENT_SECRET) || "" as string)
      .then((googleClientSecret) => {
        const isGoogleAuthenticationEnabled: boolean = !!this._serverUrl && !!googleClientId && !!googleClientSecret;

        if (isGoogleAuthenticationEnabled) {
          this.setupGoogleRoutes(router, googleClientId, googleClientSecret);
        }
      });
    });
  ...
  private setupGoogleRoutes(router: Router, googleClientId: string, googleClientSecret: string): void {
    ...
    passport.use(
      new GoogleStrategy(
        ...
        // google oauth takes done callback as 5th parameter, not like other strategies
